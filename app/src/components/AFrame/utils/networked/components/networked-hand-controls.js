...
AFRAME.registerComponent("networked-hand-controls", {
			...
			.then(() => {
				if (this.local) {
					...
					for (const evtName in this.buttonEventMap) {
						this.eventFunctionMap[this.data.hand][evtName] = this.handleButton.bind(
							this,
							...this.buttonEventMap[evtName],
						);
					}
					for (const evtName in this.visibleListeners) {
						this.eventFunctionMap[this.data.hand][evtName] =
							this.visibleListeners[evtName].bind(this);
					}
				} else {
	...
	update(oldData) {
		...
		if (!this.local) {
			if (
				this.data.handModelStyle !== this.str.controller &&
				this.data.gesture !== oldData.gesture
			) {
				...
				if (
					oldData.controllerEvent &&
					oldData.controllerEvent !== this.data.controllerEvent
				) {
					const controllerEvent = JSON.parse(this.data.controllerEvent);
					this.el.components[this.data.controllerComponent][
						controllerEvent.type === "thumbstickmoved"
							? "onThumbstickMoved"
							: "onButtonChanged"
					](controllerEvent);
				}
				if (
					oldData.controllerModelUpdate &&
					...
					this.el.components[this.data.controllerComponent].updateModel(
						...this.data.controllerModelUpdate,
					);
				}
			}
		}
	...
	addHandModel() {
		...
		const handmodelUrl =
			this.MODEL_BASE +
			this.MODEL_NAMES[
				this.data.handModelStyle +
					this.data.hand.charAt(0).toUpperCase() +
					this.data.hand.slice(1)
			];
	...
	addControllerComponents(useControllerModel) {
		...
		this.controllerComponents.forEach((controllerComponentName) => {
			if (
				this.data.controllerComponent &&
				this.data.controllerComponent !== controllerComponentName
			) {
				return;
			}

			this.el.setAttribute(controllerComponentName, {
				hand: this.data.hand,
			...
			this.el.components[controllerComponentName].injectTrackedControls =
				this.injectControlsWrapper.bind(
					this,
					this.el.components[controllerComponentName].injectTrackedControls.bind(
	...
	injectControlsWrapper(originalFn, controllerComponentName, webxrControllerSymbol) {
		...
		this.el.components[controllerComponentName].updateModel = this.updateModelWrapper.bind(
			this,
	...
	onButtonChangedWrapper(originalFn, evt) {
		this.el.setAttribute(
			...
			JSON.stringify({detail: evt.detail, type: evt.type}),
		);
		originalFn(evt);
	},
	...
	updateModelWrapper(originalFn, buttonName, evtName) {
		...
		this.el.setAttribute(
			"networked-hand-controls",
			"controllerModelUpdate",
			this.btnEvtMap[buttonName][evtName],
		);
	...
	emitGestureEvents() {
		this.el.emit(this.eventNames[this.lastGesture]?.inactive);
		this.el.emit(this.eventNames[this.data.gesture]?.active);
	},
