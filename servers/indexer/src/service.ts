...
export async function service() {
    ...
    logger.info(
        `Connecting to: ${process.env.MONGODB_URI}/${process.env.MONGODB_DATABASE}`
    );
    ...
    contract.on(
        "MarketItemCreated",
        (tokenId, creator, seller, owner, price, sold) => {
            ...
            MarketItem.findOne(
                ...
                function (err: Error, existingToken: any) {
                    ...
                    if (!existingToken) {
                        ...
                        existingToken.price.toString() !==
                            item.price.toString() ||
                        existingToken.sold !== item.sold ||
                        existingToken.owner.toLowerCase() !==
                            item.owner.toLowerCase() ||
                        existingToken.seller.toLowerCase() !==
...
const startConnection = () => {
    const provider = new ethers.providers.WebSocketProvider(
        (isDev
            ? process.env.CHAIN_TESTNET_WSS
            : process.env.CHAIN_MAINNET_WSS) || ""
    );
